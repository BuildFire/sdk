...
"use strict";$app.controller("userProfileCtrl",["Upload","$scope","StringResource",function(t,s,r){(window.authAPI.userProfileCtrlScope=s).user=null,s.userProfile=null,s.tempUser=null,s.tempUserProfile=null,s.editingProfile=!1,s.uploadProfileImage=null,s.uploadBackgroundImage=null,s._appRoot=window._appRoot;var i=window.appContext.currentApp.appId,o="appearance",e={appId:window.appContext.currentApp.appId,pluginId:o,instanceId:o},n=new AppDatastoreAPI(i),a=new window.DatastoreAPI(e);n.search({tag:"$$badges",obj:{sort:{rank:1}}},function(e,r){if(e)return console.error(e);s.badges=r,s.$$phase||s.$apply()});var l=function(e){var o=e.tags?e.tags[i]:[];if(!o.length)return[];var n=[];return s.badges.forEach(function(r){var e=o.find(function(e){return e.tagName===r.data.tag&&e.appliedCount>=r.data.tagCount});if(e){var t=Math.floor(e.appliedCount/r.data.tagCount);r.data.appliedCount=t,imageTools&&(r.data.imageUrl=imageTools.cropImage(r.data.imageUrl,{size:"xxs",aspect:"1:1",compression:80,disablePixelRatio:1})),n.push(r.data)}}),n},u=document.getElementsByClassName("profile-container")[0],p=function(){if(s.user.imageUrl)s.user.imageUrl=imageTools.cropImage(s.user.imageUrl,{size:"s",aspect:"1:1"});else{var e={id:1,method:"users/getUserImage",params:{userId:s.user._id,externalAppId:window.currentAppId}},r=window.siteConfig.endPoints.authHost+"/src/server.js?data="+encodeURIComponent(JSON.stringify(e));s.user.imageUrl=r}return s.user.imageUrl};s.getUserProfile=function(){var e={id:1,method:"users/getUserProfile",params:{userId:s.user._id}},r=window.siteConfig.endPoints.socialHost+"/src/server.js?callback=JSON_CALLBACK&data="+encodeURIComponent(JSON.stringify(e));window.bfUtils.http.jsonp(r,{bypassInterceptorForStatus:404}).success(function(e){e&&e.result&&(e.result.email=s.user.email,s.userProfile=e.result)}).error(function(e,r,t,o){s.invalidCredentials=!0,console.error(e)})},s.close=function(e){if(e||!s.busy){if(s.tempUser=null,s.tempUserProfile=null,s.editingProfile=!1,s.uploadProfileImage=null,s.uploadBackgroundImage=null,s.busy=!1,e)return s.user=null,void(s.userProfile=null);u.addEventListener("animationend",function(){s.user=null,s.userProfile=null,s.$$phase||s.$apply(function(){u.classList.remove("closing")})},{once:!0}),requestAnimationFrame(function(){u.classList.add("closing")})}},s.openProfile=function(){return!(window.location.hash="/editUserProfile")},s.openUrl=function(e){return e&&(e.toLowerCase().indexOf("http")<0&&(e="https://"+e),window.open(e,"_blank","location=yes,enableViewportScale=yes")),!1};var d=function(e,n){var r={id:1,method:"users/getPublicUser",params:{userId:e,externalAppId:window.currentAppId}},t=window.siteConfig.endPoints.authHost+"/src/server.js?callback=JSON_CALLBACK&data="+encodeURIComponent(JSON.stringify(r));window.bfUtils.http.jsonp(t,{bypassInterceptorForStatus:404}).success(function(e){e.result.badges=l(e.result),n(null,e)}).error(function(e,r,t,o){console.error(e),n(e,null)})},c=function(e,n){var r={id:1,method:"users/getUserByAccessToken",params:{accessToken:e,externalAppId:window.currentAppId}},t=window.siteConfig.endPoints.authHost+"/src/server.js?callback=JSON_CALLBACK&data="+encodeURIComponent(JSON.stringify(r));window.bfUtils.http.jsonp(t,{bypassInterceptorForStatus:404}).success(function(e){if(e&&e.error)if(1311===e.error.code){window.openDialog({data:{title:"Login Required",body:"Please login again",disableCancel:!0}},function(){authAPI.logout()})}else 1309===e.error.code?authAPI.logout():(console.error(e.error),n(e.error,null));else e.result.badges=l(e.result),n(null,e)}).error(function(e,r,t,o){console.error(e),n(e,null)})},f=null,m=null;s.open=function(e){window.spinner.show(),f=e,m=authAPI.getCurrentUser(),f||m&&m._id&&(f=m._id),f&&a.get({tag:o,withDynamicData:!0},function(e,t){if(e)return console.error(e),void callback(e);t&&t.data&&t.data.customRegistration&&t.data.customRegistration.active&&"enterprise"===window.appContext.currentApp.config.type?(s.showCustomUserProfile=!0,m&&m._id==f?c(m.accessToken,function(e,r){r&&(s.user=r.result,window.spinner.hide(),m&&m._id==s.user._id&&(s.user.userToken=m.userToken),p(),g(t.data.customRegistration,r.result),s.customUserProfile=r.result,s.getUserProfile())}):d(f,function(e,r){r&&(s.user=r.result,window.spinner.hide(),p(),g(t.data.customRegistration,r.result),s.customUserProfile=r.result,s.getUserProfile())})):m&&m._id==f?c(m.accessToken,function(e,r){r&&(s.user=r.result,window.spinner.hide(),m&&m._id==s.user._id&&(s.user.userToken=m.userToken),p(),s.getUserProfile())}):d(f,function(e,r){r&&(s.user=r.result,window.spinner.hide(),p(),s.getUserProfile())})})};var g=function(e,r){customRegistration.getFields({defaults:r,isProfileMode:!0,customRegistrationSettings:e,refreshUI:function(){s.$$phase||s.$apply()}},function(e,r){r&&(s.customRegistrationFields=r.fields,s.customRegistration=r,s.initCustomRegistration=function(){r.initializers.forEach(function(e){e()})},s.$$phase||s.$apply())})};authAPI.onLogout(function(){s.user&&delete s.user.userToken,s.close(!0)}),s.tempUser=null,s.tempUserProfile=null,s.busy=!1,s.cancelEdit=function(){return s.tempUser=null,s.tempUserProfile=null,s.editingProfile=null,s.uploadBackgroundImage=null,s.uploadProfileImage=null,s.$$phase||s.$apply(),!1};var U=function(r,n){var e={id:1,method:"users/update",params:r},t=window.siteConfig.endPoints.authHost+"/src/server.js?callback=JSON_CALLBACK&data="+encodeURIComponent(angular.toJson(e));window.bfUtils.http.jsonp(t).success(function(e){s.tempUser.lastUpdated=(new Date).toISOString(),userTags.setRegistrationTags(r.userProfile,!1,function(e,r){e&&console.log(e)}),n(null,e)}).error(function(e,r,t,o){console.error(e),s.busy=!1,n(e,null)}).finally()};s.update=function(){s.tempUserProfile||(s.tempUserProfile={}),s.busy=!0,s.tempUser.displayName=s.tempUser.firstName+" "+s.tempUser.lastName,s.uploadImage(s.uploadProfileImage,function(e,r){r&&(s.tempUser.imageUrl=r.result),s.uploadImage(s.uploadBackgroundImage,function(e,r){r&&(s.tempUserProfile.backgroundImage=r.result),function(n){if(s.customRegistration)s.customRegistration.fetch({validateFields:!0,validateLinks:!1,mode:"edit"},function(e,r){if(e)s.busy=!1;else{for(var t in r.user)r.user.hasOwnProperty(t)&&(s.tempUser[t]=r.user[t]);var o=r.user;o._id=s.tempUser._id,o.userToken=s.tempUser.userToken,o.imageUrl=s.tempUser.imageUrl,o.externalAppId=window.appContext.currentApp.appId,U(o,n)}});else{var e={};e._id=s.tempUser._id,e.email=s.tempUser.email,e.userToken=s.tempUser.userToken,e.firstName=s.tempUser.firstName,e.lastName=s.tempUser.lastName,e.displayName=s.tempUser.displayName,e.imageUrl=s.tempUser.imageUrl,e.externalAppId=window.appContext.currentApp.appId,U(e,n)}}(function(){!function(n){var e=JSON.parse(JSON.stringify(s.tempUserProfile));e||(e={}),e.userId=s.tempUser._id,e.userToken=s.tempUser.userToken,e.email=s.tempUser.email,e.firstName=s.tempUser.firstName,e.lastName=s.tempUser.lastName,e.displayName=s.tempUser.displayName;var r={id:1,method:"users/saveUserProfile",params:e},t=window.siteConfig.endPoints.socialHost+"/src/server.js?callback=JSON_CALLBACK&data="+encodeURIComponent(JSON.stringify(r));window.bfUtils.http.jsonp(t,{bypassInterceptorForStatus:404}).success(function(e){n(null,e)}).error(function(e,r,t,o){console.error(e),n(e,null)})}(function(){s.tempUser.badges=s.user.badges,s.user=s.tempUser,s.tempUser=null,s.userProfile=s.tempUserProfile,s.tempUserProfile=null,s.uploadBackgroundImage=null,s.uploadProfileImage=null,authAPI._setCurrentUser(s.user),authAPI.triggerOnUpdate(),s.editingProfile=!1,s.busy=!1})})})})},s.uploadImage=function(e,n){if(e){var r=window.siteConfig.endPoints.authHost+"/src/server.js?method=users/uploadImage";t.upload({url:r,data:{file:e,userToken:s.user.userToken,externalAppId:window.currentAppId}}).progress(function(e){s.progressMessage=!0;var r=parseInt(100*e.loaded/e.total);console.log("progress: "+r+"%")}).success(function(e,r,t,o){console.log("file uploaded."),n&&n(null,e)}).error(function(e){n&&n(e,null)}).finally(function(){})}else n(null,null)},s.validateImage=function(e,o){if(!e)return!1;if(e.type&&["image/jpeg","image/png","image/jpg"].indexOf(e.type)<0)return r.getData().then(function(e){var r=("coverPhoto"===o?"Cover":"Profile")+" photo does not support this file type.",t={richContent:'<p class="padding-top-fifteen">'+(e["coverPhoto"===o?"coverImageTypeInvalid":"profileImageTypeInvalid"]||r)+"</p>"};richModal.display(t)}),!1;return!(e.size&&1e7<e.size)||(r.getData().then(function(e){var r=("coverPhoto"===o?"Cover":"Profile")+" photo must be less than 10mb.",t={richContent:'<p class="padding-top-fifteen">'+(e["coverPhoto"===o?"coverPhotoLess10mb":"profilePhotoLess10mb"]||r)+"</p>"};richModal.display(t)}),!1)},s.editProfile=function(){null!=m&&m._id&&(s.tempUser=m,s.currentUserEmail=m.email,s.tempUserProfile=s.userProfile,s.editingProfile=!0,s.busy=!1,s.$$phase||s.$apply())},s.enforceMaxLength=function(e,r){e.value.length>r&&(e.value=e.value.substring(0,r)),e.change&&e.change(e.value),e.validate&&e.validate(e.value)},(bfUtils.isIOS||bfUtils.isAndroid)&&(u.style.maxHeight=window.innerHeight+"px",u.style.minHeight=window.innerHeight+"px")}]),$app.filter("cropCoverImage",function(){return function(e){if(!e)return"";if(!imageTools||!/(https|http):\/\//g.test(e))return e;var r="full_width";return-1<window.location.href.indexOf("pluginTester")&&(r="xl"),imageTools.cropImage(e,{size:r,aspect:"16:9",compression:80})}});
