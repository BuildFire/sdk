  ...
  editor.addButton("actionitem", {
    ...
    onclick: function () {
      ...
      buildfire.actionItems.showDialog(
        actionItem ? actionItem : { title: selection ? selection : 'Action Item' },
        ...
        (err, actionItem) => {
          ...
          if (isEditing) {
            ...
            selectedNode.innerHTML = actionItem.title;
          } else {
            let anchor = `<a href="#" data-action-item="${stringifiedActionItem}" onclick="buildfire.actionItems.execute(JSON.parse(unescape('${stringifiedActionItem}')), ()=>{})">
            ${selection ? selection : (actionItem.title ? actionItem.title : 'Action Item')}</a>`;
