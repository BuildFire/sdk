tinymce.PluginManager.add("bf_imagelib", function (editor, url) {
  ...
  editor.ui.registry.addContextToolbar('editBfImage', {
    ...
    items: 'bf_edit_image | bf_edit_image_action',
    position: 'node',
    scope: 'node'
  });
  ...
  editor.ui.registry.addContextMenu('bf_imageContextMenu', {
    update: function (element) {
      return element.dataset.bfImage ? 'bf_editImage | bf_editImageAction' : '';
    }
  });
  ...
  function showDialog(isEditing) {
    ...
    function showTinymceImageDialog(title, buttons, querystring) {
      editor.windowManager.openUrl({
        ...
        onAction: (dialogApi, details) => {
          ...
          } else if (details.name === 'Select Action') {
            dialogApi.sendMessage({
              message: 'getImageToInsertAction'
            });
          }
        },
        onMessage : (dialogApi, details) => {
          ...
          if (imageData && mceAction === 'setImage') {
            ...
            dialogApi.close();
          } else if (imageData && mceAction === 'setImageAction') {
            let options = {};
    ...
    if (!isEditing) {
      ...
      buildfire.imageLib.showDialog(options, (err, results) => {
        ...
        let buttons = [cancelButton, selectActionButton, insertImageButton]
    
        let stylesToInject = `
        button[title="Insert Image"].tox-button {
          background-color: #14cb5d;
          border-color: #14cb5d;
        }
        button[title="Insert Image"].tox-button:hover {
          background-color: #00af43 !important;
          border-color: #00af43 !important;
        }`;
