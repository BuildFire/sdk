tinymce.PluginManager.add('bf_imagelib', function (editor, url) {
    ...
    function showTinymceImageDialog(title, buttons, querystring) {
      editor.windowManager.openUrl({
        ...
        onMessage : (dialogApi, details) => {
          ...
          if (imageData.enablePreview) {
            image.setAttribute('data-original-src', imageData.originalSrc);
            image.setAttribute('onclick', 'buildfire.mediaPreviewer.show({media: [{imageSrc: this.dataset.originalSrc}]})');
          }
          if (imageData && mceAction === 'setImage') {
            if (isEditing) {
              ...
              if (imageData.enablePreview) {
                elementNode.setAttribute('data-original-src', imageData.originalSrc);
                elementNode.setAttribute('onclick', 'buildfire.mediaPreviewer.show({media: [{imageSrc: this.dataset.originalSrc}]})');
              } else {
                // Only remove onclick if it's the media previewer onclick
                const currentOnclick = elementNode.getAttribute('onclick');
                if (currentOnclick && currentOnclick.includes('buildfire.mediaPreviewer.show')) {
                  elementNode.removeAttribute('onclick');
                  elementNode.removeAttribute('data-original-src');
                }
              }
              elementNode.removeAttribute('width');
  ...
  function showActionItemDialog(options, callback) {
    options = options || {};
    ...
    buildfire.actionItems.showDialog(
      ...
      (err, actionItem) => {
        ...
        // Check if element has data-bf-image and remove enablePreview if it exists
        if (options.element.dataset.bfImage) {
          let imageData = JSON.parse(unescape(options.element.dataset.bfImage));
          if (imageData.enablePreview) {
            delete imageData.enablePreview;
            options.element.setAttribute('data-bf-image', escape(JSON.stringify(imageData)));
          }
        }
        
        // Remove data-original-src if it exists since action item overrides preview
        if (options.element.dataset.originalSrc) {
          options.element.removeAttribute('data-original-src');
        }
  ...
  function editAction() {
    ...
    showActionItemDialog(options, (err, res) => {
      if (err) return console.error(err);
      editor.isNotDirty = false;
      editor.fire('change');
    })
  ...
  function changeImage() {
    ...
    buildfire.imageLib.showDialog(options, (err, results) => {
      ...
      const size = imageProperties.fixed ? imageProperties.fixed : imageProperties.responsive;
      if (imageProperties.resize) {
        widgetImageExpression = '${buildfire.imageLib.resizeImage(\'' + escapedImageUrl + '\', { size: \'' + size + '\', aspect: \'1:1\' })}';
      } else {
        widgetImageExpression = '${buildfire.imageLib.cropImage(\'' + escapedImageUrl + '\', { size: \'' + size + '\', aspect: \'' + imageProperties.crop + '\'' +  '})}';
      }
      ...
      if (size?.includes('_') && imageProperties.resize) elementNode.setAttribute('class', size);
      elementNode.setAttribute('data-bf-image', stringifiedImageProperties);
      if (imageProperties.enablePreview) {
        elementNode.setAttribute('data-original-src', imageUrl);
      }
