tinymce.PluginManager.add('bf_videolib', function (editor, url) {
  let dialogHeight = window.innerHeight > 485 ? 485 : window.innerHeight - 20;
  ...
  editor.ui.registry.addContextToolbar('editBfVideo', {
    predicate: function (node) {
      if (node.dataset && node.dataset.bfVideo) return true;
      if (node.getAttribute && node.getAttribute('data-mce-p-data-bf-video')) return true;
      var videoChild = node.querySelector && (node.querySelector('video[data-bf-video]') || node.querySelector('iframe[data-bf-video]'));
      return !!videoChild;
    },
    items: 'bf_edit_video',
    position: 'node',
  ...
  editor.ui.registry.addContextMenu('bf_videoContextMenu', {
    update: function (element) {
      ...
      while (node && node !== editor.getBody()) {
        if (node.dataset && node.dataset.bfVideo) return 'bf_editVideo';
        if (node.getAttribute && node.getAttribute('data-mce-object')) return 'bf_editVideo';
        if (node.getAttribute && node.getAttribute('data-mce-p-data-bf-video')) return 'bf_editVideo';
        var videoChild = node.querySelector && (node.querySelector('video[data-bf-video]') || node.querySelector('iframe[data-bf-video]'));
        if (videoChild) return 'bf_editVideo';
