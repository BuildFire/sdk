...
buildfire.components.carousel.view.prototype = {
    mergeSettings: function (options) {
        var interval=(!this.control||!this.control.settings||!this.control.settings.speed)?5 * 1000:this.control.settings.speed;
    ...
    _getHoverTextStyles:function(callback){
        ...
        buildfire.appearance.getAppTheme((err,theme)=>{
            let background="black",text="white";
            ...
            textStyle=`height:13px; background-color:`+background+`; opacity:0.8; font-size:100%; padding:3.5% 2% 3.5% 2%; max-width: 95% !important; display: inline-block; 
            ...
            if(this.control.text.position==0){spanStyle+=" top:calc(2% + 8px);";}
            else if(this.control.text.position==2){spanStyle+=" bottom:calc(2% + 8px);"}
            else {spanStyle+=" top:calc(50% - 8px);";}
    ...
    _moveTimer:function(){
        ...
        if (this.config.loop && this.control.settings.display==0 && this.config.items && this.config.items.length > 1) {
            var self = this;
            if (this.timerInterval) {
                clearInterval(this.timerInterval);
                this.timerInterval = null;
    ...
    _loadSettings:function(options,callback){
        ...
        buildfire.datastore.get('carouselSettings',function(err,response){
            if(err || !response || !response.data || !response.data.text)
                response={data:{settings:{speed:5000,order:0,display:0},
