		...
		_setFontUrl: function (context, appTheme) {
			if (appTheme.fontId) {
				if (appTheme.isCustomFont) {
					appTheme.fontUrl = `${context.endPoints.pluginRootHost}/${appTheme.fontFolderPath}/${context.appId}/${encodeURIComponent(appTheme.fontFileName)}?cache-buster=${appTheme.fontLastUpdatedOn}`;
				} else {
